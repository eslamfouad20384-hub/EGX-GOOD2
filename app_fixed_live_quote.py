...
def get_current_market_price(symbol):
    """جلب آخر سعر مع التحقق من مصدره وحداثته.

    مهم: لا نعتبر آخر Close التاريخي سعر Live. لو لم يتوفر Quote حديث
    نرجع valid=False وبالتالي السهم لا يدخل التحليل النهائي.
    """
    ...
    try:
        ...
        # 1) Yahoo quote metadata. نحاول حقول السعر المباشر فقط.
        for getter_name in ("fast_info", "info"):
            try:
                data = getattr(ticker, getter_name)
                if getter_name == "fast_info":
                    data = data
                if data:
                    for key in ("last_price", "regularMarketPrice", "regular_market_price"):
                        try:
                            ...
                            if np.isfinite(value):
                                candidates.append((value, f"yahoo_{getter_name}_{key}"))
                                break
                        except Exception:
                            pass
            except Exception:
        ...
        # 2) Intraday latest bar. ده أفضل من daily history كمرجع حديث.
        try:
            intraday = ticker.history(period="1d", interval="1m", auto_adjust=False, prepost=False)
            if intraday is not None and not intraday.empty:
                closes = intraday["Close"].dropna()
                if not closes.empty:
                    value = _num(closes.iloc[-1])
                    if np.isfinite(value):
                        candidates.insert(0, (value, "yahoo_intraday_1m"))
        except Exception:
            pass

        if candidates:
            price, source = candidates[0]
            return {
                "price": float(price),
                "source": source,
                "valid": True,
                "quote_type": "LIVE_OR_LATEST_QUOTE",
...
def process(
    ...
    try:
        ...
        # =================================================
        live_quote = get_current_market_price(symbol)

        if not live_quote.get("valid", False):
            return {
                "السهم": clean_symbol,
                "الحالة": "❌ تعذر جلب Quote حديث؛ تم منع Entry/Stop/Targets النهائية"
            }
        ...
        if not np.isfinite(current_price) or current_price <= 0:
            return {
                "السهم": clean_symbol,
                "الحالة": "❌ السعر المستلم غير صالح؛ تم منع التحليل النهائي"
            }
