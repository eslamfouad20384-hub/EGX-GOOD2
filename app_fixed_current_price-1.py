...
# =========================================================
# 📡 LIVE CURRENT PRICE ENGINE
# =========================================================

def _normalize_egx_symbol(symbol):
    return str(symbol).upper().replace('.CA', '').strip()


def _parse_eod_payload(payload):
    """Extract date/close from common EOD JSON shapes."""
    rows = payload
    if isinstance(payload, dict):
        for key in ("data", "results", "prices", "eod", "historical"):
    ...
    for row in rows:
        if not isinstance(row, dict):
            continue
        date_value = row.get("date") or row.get("Date") or row.get("datetime") or row.get("timestamp")
...
def _get_eodhd_latest_close(symbol):
    """Optional EODHD fallback. Token is read from Streamlit secrets/env."""
    import os
    import requests

    token = None
    try:
        token = st.secrets.get("EODHD_API_TOKEN")
    except Exception:
        pass
    token = token or os.getenv("EODHD_API_TOKEN")
    if not token:
        return None

    ticker = f"{_normalize_egx_symbol(symbol)}.EGX"
    url = f"https://eodhd.com/api/eod/{ticker}"
    try:
        r = requests.get(url, params={"api_token": token, "fmt": "json", "order": "d", "limit": 10}, timeout=8)
        r.raise_for_status()
        date_value, price = _parse_eod_payload(r.json())
        if date_value is not None and np.isfinite(price) and price > 0:
            return {"price": float(price), "date": date_value, "source": "EODHD_EOD"}
    except Exception:
        pass
    return None


def _get_egx_news_latest_close(symbol):
    """Optional EGX.news adapter. Configure EGX_NEWS_API_URL in secrets/env.
    The URL may contain {symbol}; otherwise symbol is sent as a query parameter.
    """
    import os
    import requests
    ...
    api_url = None
    try:
        api_url = st.secrets.get("EGX_NEWS_API_URL")
    except Exception:
        pass
    api_url = api_url or os.getenv("EGX_NEWS_API_URL")
    if not api_url:
        return None

    clean = _normalize_egx_symbol(symbol)
    try:
        if "{symbol}" in api_url:
            url = api_url.format(symbol=clean)
            r = requests.get(url, timeout=8)
        else:
            r = requests.get(api_url, params={"symbol": clean}, timeout=8)
        r.raise_for_status()
        date_value, price = _parse_eod_payload(r.json())
        if date_value is not None and np.isfinite(price) and price > 0:
            return {"price": float(price), "date": date_value, "source": "EGX.news_EOD"}
    except Exception:
        pass
    return None


def get_current_market_price(symbol, expected_date=None):
    """
    يجلب آخر إغلاق يومي مكتمل بشكل مستقل عن مصدر الشموع.
    الأولوية: EGX.news ثم EODHD.
    لا يتم استخدام Close التاريخي من df كبديل صامت، ولا يتم اعتبار
    سعر Yahoo اللحظي سعراً صحيحاً بعد إغلاق EGX.
    """
    candidates = []

    egx = _get_egx_news_latest_close(symbol)
    if egx:
        candidates.append(egx)

    eodhd = _get_eodhd_latest_close(symbol)
    if eodhd:
        candidates.append(eodhd)

    if not candidates:
        return {"price": np.nan, "date": None, "source": "unavailable", "valid": False,
                "reason": "لم يتم ضبط مصدر EOD مستقل (EGX.news/EODHD) أو تعذر الوصول إليه"}

    # لو عرفنا تاريخ آخر شمعة مكتملة، لا نقبل سعراً أقدم منه.
    if expected_date is not None:
        expected = pd.to_datetime(expected_date, errors="coerce")
        if not pd.isna(expected):
            expected = expected.date()
            fresh = [x for x in candidates if x.get("date") == expected]
            if fresh:
                x = fresh[0]
                return {**x, "valid": True, "date": str(x["date"])}
            newer = [x for x in candidates if x.get("date") and x["date"] > expected]
            if newer:
                x = sorted(newer, key=lambda z: z["date"])[-1]
                return {**x, "valid": True, "date": str(x["date"])}
            return {"price": np.nan, "date": None, "source": "stale", "valid": False,
                    "reason": f"مصدر السعر أقدم من آخر جلسة متاحة ({expected})"}

    x = sorted(candidates, key=lambda z: z.get("date") or pd.Timestamp.min.date())[-1]
    return {**x, "valid": True, "date": str(x["date"])}
...
def determine_entry(
    ...
    # القرار النهائي يعتمد على آخر سعر حالي، وليس Close تاريخي فقط.
    historical_close = float(last["Close"])
    try:
        market_price = float(current_price)
    except Exception:
        market_price = historical_close
...
def analyze(
    ...
    historical_close = float(last_d["Close"])

    # لا نكمل النتيجة النهائية بدون سعر حالي صالح.
    try:
        market_price = float(current_price)
    except Exception:
        market_price = np.nan
    ...
    if not np.isfinite(market_price) or market_price <= 0:
        raise ValueError("❌ لم يتم الحصول على آخر سعر حالي موثوق")
    ...
    return {
        ...
        "السعر الحالي": round(market_price, 2),
        "آخر إغلاق للبيانات": round(historical_close, 2),
        "فرق السعر الحالي عن آخر إغلاق %": round((market_price / historical_close - 1) * 100, 2) if historical_close > 0 else np.nan,
        "مصدر السعر الحالي": current_price_source,
        "تاريخ السعر الحالي": current_price_date,
...
def process(
    ...
    try:
        ...
        # =================================================
        # LIVE PRICE GATE
        # لا يتم حساب Entry / Stop / Targets النهائية إلا بعد
        # الحصول على آخر سعر حالي مستقل عن بيانات الشموع.
        # =================================================
        expected_eod_date = df_d.index[-1] if not df_d.empty else None
        live_quote = get_current_market_price(symbol, expected_date=expected_eod_date)

        if not live_quote.get("valid", False):
            return {
                "السهم": clean_symbol,
                "الحالة": "❌ تعذر جلب السعر الحالي؛ تم منع التحليل النهائي"
            }
        ...
        result = analyze(
            ...
            current_price_source=live_quote.get("source", "unknown"),
            current_price_date=live_quote.get("date")
        )
        if run_backtest:
