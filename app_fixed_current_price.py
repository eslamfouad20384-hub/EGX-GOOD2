...
def get_current_market_price(symbol):
    """
    يجلب آخر سعر متاح من Yahoo Finance بشكل مستقل عن بيانات الشموع
    التاريخية. لا نستخدم سعر الـ Close التاريخي كبديل صامت.
    """
    try:
        ...
        # history كـ fallback فقط لو fast_info فشل.
        if not candidates:
            try:
                ...
                if hist is not None and not hist.empty:
                    ...
                    if np.isfinite(value) and value > 0:
                        candidates.append(("history_last_close", value))
            except Exception:
                pass
